...
namespace ESBasic.Security
{
    ...
    public class DesEncryption
    {
        ...
        public static string EncryptString(string origin, string key)
        {
            ...
            byte[] res = DesEncryption.Encrypt(buff, key);
        ...
        #region DesStrategy
        private DesStrategy desStrategy = DesStrategy.DesSimple;
        /// <summary>
        /// 加密策略
        /// </summary>
        public DesStrategy DesStrategy
        {
            get { return desStrategy; }
            set { desStrategy = value; }
        }
        #endregion
        #endregion
        ...
        public DesEncryption(DesStrategy strategy, string _key)
        {
        ...
        /// Ctor。
        /// </summary>
        /// <param name="strategy">加密策略</param>
        /// <param name="_key">密匙</param>
        /// <param name="_iniVector">如果加密方式是DesCBC模式，_iniVector表示初始向量</param>
        public DesEncryption(DesStrategy strategy, string _key, string _iniVector)
        {
            this.desStrategy = strategy;
            this.key = _key;
        ...
        public byte[] Encrypt(byte[] origin)
        {
            ...
            byte[] des_bytes = null;
            if (this.desStrategy == DesStrategy.DesSimple)
            {
                des_bytes = this.Des(bts2, key, true);
            }
            else if (this.desStrategy == DesStrategy.Des3)
            {
                des_bytes = this.Des3(bts2, key, true);
            }
            else if (this.desStrategy == DesStrategy.DesCBC)
            {
                des_bytes = this.DesCBC(bts2, key, this.iniVector, true);
            }
            else if (this.desStrategy == DesStrategy.DesTwoKeys)
            {
                des_bytes = this.DesTwoKeys(bts2, key, this.iniVector, true);
            }
        ...
        public byte[] Decrypt(byte[] encrypted)
        {
            ...
            byte[] enDes_bytes = null;
            if (this.desStrategy == DesStrategy.DesSimple)
            {
                enDes_bytes = this.Des(encryptedAll, this.key, false);
            }
            else if (this.desStrategy == DesStrategy.Des3)
            {
                enDes_bytes = this.Des3(encryptedAll, this.key, false);
            }
            else if (this.desStrategy == DesStrategy.DesCBC)
            {
                enDes_bytes = this.DesCBC(encryptedAll, this.key, this.iniVector, false);
            }
            else if (this.desStrategy == DesStrategy.DesTwoKeys)
            {
                enDes_bytes = this.DesTwoKeys(encryptedAll, this.key, this.iniVector, false);
            }
    ...
    public enum DesStrategy
    {
        DesSimple, Des3, DesCBC, DesTwoKeys
    }
}
